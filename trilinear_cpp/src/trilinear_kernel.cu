...
__global__ void TriLinearForward(const int nthreads, const float* lut, const float* image, float* output, const int dim, const int shift, const float binsize, const int width, const int height, const int batch) {
        ...
        const int totalLut = dim * dim * dim;
        const int max_id   = dim - 1;
        ...
        r_id = r_id < 0 ? 0 : (r_id > safe_max ? safe_max : r_id);

        int g_id = (int)floorf(g / binsize);
        g_id = g_id < 0 ? 0 : (g_id > safe_max ? safe_max : g_id);

        int b_id = (int)floorf(b / binsize);
        b_id = b_id < 0 ? 0 : (b_id > safe_max ? safe_max : b_id);

        float r_d = (r - r_id * binsize) / binsize;
        float g_d = (g - g_id * binsize) / binsize;
        float b_d = (b - b_id * binsize) / binsize;
